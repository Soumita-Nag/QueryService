<template>
  <div class="w-full max-w-2xl mx-auto space-y-4">
   <div @click="showAnswer" class="cursor-pointer space-y-4 bg-white shadow-md rounded-xl p-4" v-for="i in 5">
    <div>
      <h1 class="text-2xl font-bold text-gray-800">{{ recentQueries.queryHead }}</h1>
      <h4 class="text-md text-gray-600 mt-2 truncate">{{ recentQueries.queryBody }}</h4>
    ...
    <div class="text-xs text-gray-500">
      {{ props.recentQueries.user }} asked on {{ props.recentQueries.date }} at {{ props.recentQueries.time }}
    </div>
   </div>
  </div>
</template>
